...
🔮 Future Improvements
[ ] Web Search Node: Integrate Tavily Search API to fetch real-time data if local context is insufficient.
[ ] Hallucination Grader: Add a "Grader Agent" to verify if the answer is supported by the documents before showing it to the user.
[ ] Streamlit UI: Build a frontend for easier interaction.
